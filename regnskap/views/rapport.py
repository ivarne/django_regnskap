...
def showYear(request, prosjekt, year):
    ...
    kostKonto= list(Konto.objects.sum_columns(int(year)).filter(kontoType__in = (4,5,6,7,8,9)))
    intKonto = list(Konto.objects.sum_columns(int(year)).filter(kontoType = 3 ))
    ...
    eiendelKonto =list(Konto.objects.sum_columns(int(year)).filter(kontoType = 1))
    finansKonto = list(Konto.objects.sum_columns(int(year)).filter(kontoType = 2))
    # adjust the lists so that they get equal lenght
